...
| --- | --- | --- |
| Fresh clone → Python install → doctor → Mock generation | verified | `.github/workflows/ci.yml` runs the clean-checkout smoke on Ubuntu/Python 3.12; `scripts/ci_smoke.py` uses a temporary TXT and no API keys/models. |
...
| Full public-domain book, manifest, disk growth, completed resume, M4B chapters | manual | `BOOKCAST_RUN_RELEASE_LARGE_BOOK=1 uv run pytest -m large_model tests/test_release_large_book.py -v`; this downloads/handles a multi-megabyte public-domain text but uses Mock providers and no paid AI. |
...
| Qwen local TTS | optional | Experimental, large local model, platform/resource-dependent. See `docs/TTS_PROVIDER_EVALUATION.md`; no PR download/inference. |
...
| Release secret scan | manual | Scan Git history, source, config examples, build outputs, and packaged artifacts before publishing. Never include `.env`, credentials, private books, models, or generated audio. |
| GitHub Actions execution on canonical GitHub repo | manual | Workflow is committed/configured; verify the first `main` push and pull request runs. No remote run is claimed until GitHub reports results. |
