...
Phase 18 的时长预算现在于全书综合前约束候选章节数；20 分钟最多 32 个沿书序分布的候选、16 个脚本/复核片段。所有净化后的来源单元仍逐块分析并保留证据；未入选单元在 `omitted_chapters` 可见，不进入脚本与语义复核。逐章综合每批最多 12 个主题。详见 [LLM 成本审计](LLM_COST.md)。

Phase 7 的 [Web 客户端](WEB.md) 将三种模式和时长预算传入同一 Core，面板可展开质量警告；不会在前端生成脚本。界面默认 40 分钟预算，Core/CLI 默认值仍是 10 分钟；Phase 8 可选 [本地中文 TTS](TTS.md)，默认 Mock 为测试音调。语音类型与内容的 is_mock 分开记录，真实人声不会消除 Mock 内容的质量提示。
