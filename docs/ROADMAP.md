...
继续扩大合法样本和真实服务的质量评估。EPUB 已增加 D-022 的高置信非正文过滤及来源审计；语义分章、未知广告识别、PDF 复杂排版、扫描页 OCR 策略及跨平台 FFmpeg 样本仍需单独验收。协议兼容性测试不等同于中文内容质量验收。
...
4. 完成零推理/零HTTP真实任务恢复验证、回归与全量离线测试，更新状态和交接；Phase 13 已另行实施。

### Phase 13：M4B / Audiobook Export

**状态：完成；小型旧 Job 和真实中文节目均通过 FFmpeg/ffprobe 验证，离线回归与 Web 浏览器 E2E 通过。**

1. 从完成 Job 的有效 MP3、真实 WAV 时长和节目规划独立导出 AAC/M4B，不调用 LLM/TTS，保留 MP3。
...
### Phase 18：LLM 成本审计与时长预算前移

**状态：离线实现与 Mock 回归完成；新真实长书的质量/计费对照待用户显式运行。**

审计现存真实任务的 manifest usage，不重调 DeepSeek。完整来源分析继续保留；逐章综合有界合批，全书综合前按时长和书序选候选，后续只对计划 segment 写作和复核。逐阶段输出预算与 `usage/llm_usage.json` 可审计总量；保留来源校验、矛盾检测、质量门禁和定向修复。离线调用数估算及限制见 [LLM_COST.md](LLM_COST.md)。

### 后续候选

**状态：规划中。**

在已实现本地中文人声基础上，试听并改进专有名词、停顿和长节目自然度。付费 Provider 只能作为用户显式选择的扩展。
