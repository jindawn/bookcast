...
更新时间：2026-09-14T14:42:38Z；当前分支 `main`。先按 AGENTS 阅读文档并核对实际代码/Git。

## 当前目标

Phase 5：长任务可靠性、断点恢复、持久化任务模型和命令。实现、自动测试、实际 CLI demo、文档一致性及功能提交验证均已完成。没有开始下一阶段。
...
- 在功能提交 `c05af03be05d50995d90940112a902c101f29135` 上重新执行完整测试：204 passed、10 subtests passed；compileall、项目校验、git show --check 均通过。CLI 六项回归在帮助文本调整后也已通过。
...
1. 核对 STATE 的已验证提交与实际 Git；阅读 JOBS 和 PHASE5_VERIFICATION，掌握恢复与缓存契约。
2. 按用户授权选择下一阶段；不要将长期路线图直接当作已授权实施范围。
...
最近已验证功能提交：`c05af03be05d50995d90940112a902c101f29135` — feat: add durable jobs and crash-safe pipeline recovery（24个文件）。已在该提交运行204项测试、10个子测试、编译、项目校验和提交空白检查，全部通过。前一阶段交接为 `977f4064c37f56b3958db812e9ecf21efda33740`。本阶段仅本地提交，远端状态接手时用 Git 核对。
